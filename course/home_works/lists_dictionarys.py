...
edited_names = [];
for name in names:
    result = {'name': name.title(), 'nameLenght': len(name)}
...
edited_nums = [];
for number in nums2:
    res = {'number': number, 'square': number ** 2, 'cube': number ** 3}
    edited_nums.append(res)
for res in edited_nums:
    print(res)
...
print(f'Longest string in list is: {longest_string}')
